"""
ACT 推理客户端 - 运行在 Isaac Sim 环境中

连接 ACT 推理服务器，发送图像和关节状态，接收并执行动作

运行命令：
cd /home/jer/ws_issac/thirdparty/IsaacLab
conda activate isaaclab
export ISAACSIM_ASSETS_PATH=~/isaacsim/Assets/Assets/Isaac/5.1
./isaaclab.sh -p /home/jer/ws_issac/ws/j_so101_sim2real_touch/src/act_inference_client.py --enable_cameras
"""
...
parser.add_argument("--server_ip", type=str, default="127.0.0.1")  # 对应服务器地址
parser.add_argument("--server_port", type=int, default=9876)  # 对应服务器端口
...
def reset_environment(cube, plate, robot, origins, sim, light_prim=None):
    ...
    # wrist_roll（第5个关节）锁定在 -1.57（-90度），与遥控录制时一致
...
class ACTClient:
    def __init__(self, server_ip, server_port):
        ...
        print(f"[客户端] 已连接推理服务器 {server_ip}:{server_port}")
...
def main():
    print("=" * 60)
    print("🚀 SO101 ACT 模型推理 - Isaac Sim 客户端")
    print("=" * 60)
    ...
    print("[连接] 正在连接推理服务器...")
    time.sleep(2)
    client = ACTClient(args.server_ip, args.server_port)
    ...
    try:
        while sim_app.is_running():
            if not manual_mode:
                ...
                try:
                    action = client.get_action(joint_state, cam1_rgb, cam2_rgb)
                    action_clipped = clip_joint_angles(action)
                    
                    joint_names_list = list(robot.data.joint_names)
                    
                    # 固定 wrist_roll（第5个关节）= -1.57（-90度，录制时锁定值）
                    ...
                    if count % 30 == 0:
                        diff = [action_clipped[i] - joint_state[i] for i in range(6)]
                        print(f"[推理] step={count}")
                        print(f"  关节名称: {joint_names_list[:6]}")
                        print(f"  当前状态: {[f'{j:.3f}' for j in joint_state[:6]]}")
                        print(f"  目标位置: {[f'{a:.3f}' for a in action_clipped[:6]]}")
