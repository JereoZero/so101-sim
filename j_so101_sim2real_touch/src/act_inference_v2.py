"""
ACT 直接推理 V2 - 在 Isaac Sim 环境中直接加载模型

Chunk 队列模式：每30步预测一次 chunk，依次执行
...
./isaaclab.sh -p /home/jer/ws_issac/ws/j_so101_sim2real_touch/src/act_inference_v2.py --enable_cameras
"""
...
def main():
    ...
    # Chunk 队列管理
    chunk_size = policy.config.chunk_size  # 30
    n_action_steps = policy.config.n_action_steps  # 30
    action_chunk = None
    chunk_idx = 0
    ...
    try:
        while sim_app.is_running():
            ...
            if count % n_action_steps == 0:
                ...
                # 预测完整 chunk
                action_tensor = policy.predict_action_chunk(obs_processed)
                action_tensor = postprocess(action_tensor)
                
                # 提取 chunk: (1, chunk_size, 6) -> (chunk_size, 6)
                action_chunk = action_tensor.squeeze(0).cpu().numpy()
                chunk_idx = 0
                
                print(f"\n[推理] step={count}, 预测新 chunk, 长度={len(action_chunk)}")

            if action_chunk is not None and chunk_idx < len(action_chunk):
                # 从 chunk 中取出当前步的动作
                target = action_chunk[chunk_idx].tolist()
                
                # 锁定 wrist_roll
                target[4] = -1.57
                
                # 执行动作
                robot.set_joint_position_target(torch.tensor([target], device=sim.device))
                chunk_idx += 1
